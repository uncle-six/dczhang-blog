...
const EmptyData = pageId => {
  const empty = {
    ...
    allPages: [
      ...
      {
      id: 'oops', // 关键：不要是数字
      title: `无法获取Notion数据，请检查 Notion_ID。\n当前 ${pageId}`,
      summary:
        '访问文档获取帮助 → https://docs.tangly1024.com/article/vercel-deploy-notion-next',
      status: 'Draft', // 建议：别 Published
      type: 'Menu',    // 关键：让它不会被当作文章解析
      slug: 'oops',
      publishDay: '2024-11-13',
      pageCoverThumbnail: BLOG.HOME_BANNER_IMAGE || '/bg_image.jpg',
      date: {
...
function getSiteInfo({ collection, block, NOTION_CONFIG, rawMetadata }) {
  ...
  // ✅ 优先用 Notion 数据库页面本身的封面(rawMetadata.format.page_cover)，其次用 collection.cover，最后兜底
  const notionPageCover = rawMetadata?.format?.page_cover
    ? mapImgUrl(rawMetadata.format.page_cover, rawMetadata, 'page')
    : null
  ...
  const pageCover = notionPageCover || collectionCover || defaultPageCover
