...
function getSiteInfo({ collection, block, NOTION_CONFIG, rawMetadata }) {
  ...
  // ✅ 1) Notion 页面自身封面（你在 Notion 页面顶部设置的 cover）
  const pageCoverFromPage =
    rawMetadata?.format?.page_cover ||
    rawMetadata?.value?.format?.page_cover ||
    null

  // ✅ 2) collection 级别的 cover（有些模板会放这里）
  const pageCoverFromCollection =
    collection?.cover ||
    collection?.value?.cover ||
    collection?.value?.value?.cover ||
    null

  // ✅ 3) 最终 pageCover：Page > Collection > 配置/默认
  let pageCover = defaultPageCover

  if (pageCoverFromPage) {
    // 注意：page_cover 属于 block/page 维度，用 block 方式 map
    pageCover = mapImgUrl(
      pageCoverFromPage,
      rawMetadata?.value || rawMetadata,
      'block'
    )
  } else if (pageCoverFromCollection) {
    // collection 维度 cover
    pageCover = mapImgUrl(
      pageCoverFromCollection,
